...
async def get_forecast(model_id: int, params: ForecastIn, request: Request, db: Session = Depends(get_db)) -> ForecastOut:
    ...
    if ForecastModels.MLPRegressor == model.type:
        #keys = params.model_input_data.keys()
        if params.customer_flow_and_team_size:
            if len(keys) != 3:
    ...
    #elif ForecastModels.HistGradientBoostingRegressor.value == model.type:

    #elif ForecastModels.Prophet.value == model.type:
