""" 
	Train a model integration web service.
...
# custom imports
from app.models.models import TrainIn, TrainTaskOut, TaskState, MessageLevel, TypeForecast, CustomerFlowModels, SizingModels, CustomerFlowAndSizingModels, ForecastingModels
...
def is_model_available(model_type: str, type_of_forecast: TypeForecast) -> bool:

    available_models = CustomerFlowModels

    if type_of_forecast == TypeForecast.Sizing.name:
        available_models = SizingModels
    elif type_of_forecast == TypeForecast.CustomerFlowAndSizing.name:
        available_models = CustomerFlowAndSizingModels
...
def get_dir(models_storage_dir: str, client_id: str, type_of_forecast: TypeForecast) -> str:
    ...
    dir = models_storage_dir + client_id# + '/' + type_of_forecast_dir

    if not os.path.exists(dir):
        os.mkdir(dir)

    dir += '/' + type_of_forecast_dir
...
def model_details_to_html(
    ...
    if model_in_db.type == ForecastingModels.MLPRegressor:
        html += '<tr>'
...
def create_html_report(
    ...
    if model_in_db.type == ForecastingModels.RandomForestRegressor or (model_in_db.type == ForecastingModels.MLPRegressor and type_of_forecast == TypeForecast.Sizing.name):
        d_metrics = {}
        ...
        html_report += to_html(data=d_metrics, title=model_metrics_elem_title, round_val=True, num_decimals=3)
    elif model_in_db.type == ForecastingModels.Prophet:
...
def train_mlp_for_sizing(params: TrainIn, task_id: int) -> dict:
    """
    Trains a MLPRegressor for sizing model.
    https://scikit-learn.org/stable/modules/generated/sklearn.neural_network.MLPRegressor.html
    """
    ...
    mlp_model_forecast = mlp_model_forecast.flatten()
    ...
    raw_model_metrics = myfuncs.model_metrics(y_test, mlp_model_forecast)
    round_model_metrics = myfuncs.model_metrics(y_test, y_pred_round)
    ceil_model_metrics = myfuncs.model_metrics(y_test, y_pred_ceil)
    floor_model_metrics = myfuncs.model_metrics(y_test, y_pred_floor)
...
def train_mlp_multivariate(params: TrainIn, task_id: int) -> dict:
    ...
    # model forecasting accurracy/evaluation step
...
def switch(model_type: str, type_of_forecast: TypeForecast, params: TrainIn, task_id: int) -> Union[dict, None]:
    """
    ...
    elif model_type == ForecastingModels.MLPRegressor and type_of_forecast == TypeForecast.Sizing.name:
        return train_mlp_for_sizing(params, task_id)        
...
def train_a_model(db: Session, client_pkey: int, model_type: str, type_of_forecast: TypeForecast, params: TrainIn, task_id: int) -> bool:
    """
	Trains a model.
	"""
    ...
    try:
        ...
        # train the model
        train_output = switch(model_type, type_of_forecast, params, task_id)
        ...
        if train_output != None:
            ...
            elif model_type == ForecastingModels.MLPRegressor and type_of_forecast == TypeForecast.CustomerFlowAndSizing.name:
            ...
            if model_type == ForecastingModels.Prophet:
            ...
            if model_type == ForecastingModels.MLPRegressor:
                model_params = schemas.ModelCreate(type = model_type \
                    ...
                    , type_forecast = type_of_forecast
		        )
            else:
                model_params = schemas.ModelCreate(type = model_type \
                    , model_name = model_name \
            ...
            html_report = create_html_report(
                ...
                client_id=client_id,
                report_dir_name=report_dir_name,
                type_of_forecast=type_of_forecast
                )
            ...
            dir = get_dir(settings.MODELS_STORAGE_DIR, client_id, type_of_forecast)
...
async def train_model_request(model_type: str, params: TrainIn, background_tasks: BackgroundTasks, db: Session = Depends(get_db)) -> dict:
    ...
    # forecast customer flow and team size simultaneously.
    elif model_type == ForecastModels.MLPRegressor2:
        if len(keys) != 3:
            log_message(f'Client {client_id} model {model_type}. Unexpected number of keys in input data structure {keys} ({len(keys)}) expected (3)!', MessageLevel.Critical)
            raise HTTPException(status_code=400, detail=f'Unexpected input data structure!')
...
@router.post('/customer/flow/models/{model_type}/train', status_code=HTTPStatus.ACCEPTED)
async def train_a_model_to_predict_customer_flow(model_type: str, params: TrainIn, background_tasks: BackgroundTasks, db: Session = Depends(get_db)) -> dict:
    ...
    # 2. check if the model type requested is available (supported)
    if not is_model_available(model_type, type_of_forecast):
        # log error
        log_message(f'Model type {model_type} not available!', MessageLevel.Critical)
    ...
    # log message
    log_message(f'Task {task.id} for client {client_id} ({client_pkey}) train a model {model_type} created!', MessageLevel.Info)

    # 4. create background task to train the model
    background_tasks.add_task(train_a_model, db, client_pkey=client_pkey, model_type=model_type, type_of_forecast=type_of_forecast, params=params, task_id=task.id)

    # 5. reply to the client (task created)
    return {'detail': '1', 'task_id': task.id}

@router.post('/sizing/models/{model_type}/train', status_code=HTTPStatus.ACCEPTED)
async def train_a_model_to_predict_sizing(model_type: str, params: TrainIn, background_tasks: BackgroundTasks, db: Session = Depends(get_db)) -> dict:
    ...
    if model_type == ForecastingModels.RandomForestRegressor:
        if len(keys) != 3:
            log_message(f'Client {client_id} model {model_type}. Unexpected number of keys in input data structure {keys} ({len(keys)}) expected (3)!', MessageLevel.Critical)
            raise HTTPException(status_code=400, detail=f'Unexpected input data structure!')
...
@router.post('/customer/flow/and/sizing/models/{model_type}/train', status_code=HTTPStatus.ACCEPTED)
async def train_a_model_to_predict_customer_flow_and_sizing_simultaneously(model_type: str, params: TrainIn, background_tasks: BackgroundTasks, db: Session = Depends(get_db)) -> dict:
