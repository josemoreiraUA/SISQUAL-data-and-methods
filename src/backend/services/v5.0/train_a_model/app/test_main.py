...
SQLALCHEMY_DATABASE_URL = "sqlite:///C:/temp/backend/services/train_a_model_test.db"
...
def test_get_list_available_forecast_customer_flow_models():
    #response = client.get(f'{request_prefix}/models/forecast/customer/flow', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
    response = client.get(f'{request_prefix}/models/customer/flow', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
...
def test_get_list_available_forecast_sizing_models():
    #response = client.get(f'{request_prefix}/models/forecast/sizing', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
    response = client.get(f'{request_prefix}/models/sizing', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})

    assert response.status_code == 200
    assert len(response.json()['models']) == 2
    assert response.json() == {'models':[{'model_type':'MLPRegressor','model_name':'MLPRegressor'},{'model_type':'RandomForestRegressor','model_name':'RandomForestRegressor'}]}

def test_get_list_available_forecast_customer_flow_and_sizing_models():
    #response = client.get(f'{request_prefix}/models/forecast/customer/flow/and/sizing', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
    response = client.get(f'{request_prefix}/models/customer/flow/and/sizing', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})

    assert response.status_code == 200
    assert len(response.json()['models']) == 1
    assert response.json() == {'models':[{'model_type':'MLPRegressor','model_name':'MLPRegressor'}]}
...
def test_train_model_non_existing_client():
    ...
    response = client.post(f'{request_prefix}/customer/flow/models/{model_type}/train', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN}, json={'client_id': client_id, 'model_name': model_name, 'input_data': input_data, 'forecast_period': forecast_period, 'n_lags': n_lags})
...
def test_train_MLPRegressor_model_for_customer_flow_forecasting():
    client_id = 'AR21TY2023'
    model_name = 'Store A Customer Flow'
...
def test_get_existing_model_details():
    ...
    assert response.json()['type_forecast'] == 'CustomerFlow'
...
def test_train_MLPRegressor_model_for_sizing_forecasting():
    client_id = 'AR21TY2023'
    model_name = 'Store A Sizing'
    ...
    response = client.post(f'{request_prefix}/sizing/models/{model_type}/train', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN}, json={'client_id': client_id, 'model_name': model_name, 'input_data': input_data, 'forecast_period': forecast_period, 'n_lags': n_lags})
...
def test_get_existing_model_details2():
    model_id = 2
    model_name = 'Store A Sizing'
    response = client.get(f'{request_prefix}/models/{model_id}', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
    ...
    assert response.json()['type'] == 'MLPRegressor'
    assert response.json()['model_name'] == model_name
    assert response.json()['type_forecast'] == 'Sizing'

def test_get_existing_task_details2():
    response = client.get(f'{request_prefix}/tasks/{task_id}', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
...
def test_train_MLPRegressor_model_with_less_than_min_data_size():
    ...
    assert response.json()['error_report'] == '<div><h3>Task 7 Error: Training data size (2159 hours) too small (must be at least 2161 hours)!</h3></div>'
...
def test_train_MLPRegressor_model_with_less_than_min_data_size_equal_zero():
    ...
    assert response.json()['state'] == 'Error'
    assert len(response.json()['error_report']) > 0, 'When finished, a task should send an error report.'
    assert response.json()['error_report'] == '<div><h3>Task 8 Error: Training data size (2160 hours) too small (must be at least 2161 hours)!</h3></div>'
...
def test_train_HistGradientBoostingRegressor_model_with_min_data_size():
    ...
    response = client.post(f'{request_prefix}/customer/flow/models/{model_type}/train', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN}, json={'client_id': client_id, 'model_name': model_name, 'input_data': input_data, 'forecast_period': forecast_period})
...
def test_train_Prophet_model_unexpected_y_input_data():
    ...
    assert response.status_code == 200
    assert response.json()['state'] == 'Error'
    assert response.json()['error_report'] == '<div><h3>Task 12 Error: can only concatenate str (not "float") to str!</h3></div>'
...
def test_train_Prophet_model_unexpected_ds_input_data():
    ...
    assert response.status_code == 200
    assert response.json()['state'] == 'Error'
    assert response.json()['error_report'] == '<div><h3>Task 13 Error: Given date string a not likely a datetime present at position 0!</h3></div>'
...
def test_train_RandomForestRegressor_model():
    ...
    response = client.post(f'{request_prefix}/sizing/models/{model_type}/train', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN}, json={'client_id': client_id, 'model_name': model_name, 'input_data': input_data, 'forecast_period': forecast_period})
...
def test_train_MLPRegressor_model_tickets_and_sizing():
    ...
    response = client.post(f'{request_prefix}/customer/flow/and/sizing/models/{model_type}/train', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN}, json={'client_id': client_id, 'model_name': model_name, 'input_data': input_data, 'forecast_period': forecast_period, 'customer_flow_and_team_size': True})
...
def test_get_client_models():
    ...
    assert response.status_code == 200
    assert len(response.json()['models']) == 10, f'{client_id} is expected to have 10 models!'

def test_get_client_customer_flow_models():
    client_id = 'AR21TY2023'
    response = client.get(f'{request_prefix}/clients/{client_id}/models/customer/flow', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
    
    assert response.status_code == 200
    assert len(response.json()['models']) == 7, f'{client_id} is expected to have 7 models!'

def test_get_client_sizing_models():
    client_id = 'AR21TY2023'
    response = client.get(f'{request_prefix}/clients/{client_id}/models/sizing', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
    
    assert response.status_code == 200
    assert len(response.json()['models']) == 2, f'{client_id} is expected to have 2 models!'

def test_get_clientcustomer_flow_and_sizing_models():
    client_id = 'AR21TY2023'
    response = client.get(f'{request_prefix}/clients/{client_id}/models/customer/flow/and/sizing', headers={'Authorization': 'Bearer ' + settings.JWT_TOKEN})
    
    print(response.json()['models'])
    print(response.json()['models'][0])

    assert response.status_code == 200
    assert len(response.json()['models']) == 1, f'{client_id} is expected to have 1 model!'
    assert response.json()['models'][0]['type_forecast'] == 'CustomerFlowAndSizing'
...
def test_get_client_tasks():
    ...
    assert response.status_code == 200
    assert len(response.json()['tasks']) == 15, f'{client_id} is expected to have 15 tasks!'