...
from app.models.models import CustomerFlowModels, SizingModels, CustomerFlowAndSizingModels
...
class Settings(BaseSettings):
    ...
    AVAILABLE_CUSTOMER_FLOW_FORECASTING_MODELS: List[dict] = []

    for model in CustomerFlowModels:
        AVAILABLE_CUSTOMER_FLOW_FORECASTING_MODELS += [{'model_type': model.name, 'model_name': model.value}]

    AVAILABLE_SIZING_FORECASTING_MODELS: List[dict] = []

    for model in SizingModels:
        AVAILABLE_SIZING_FORECASTING_MODELS += [{'model_type': model.name, 'model_name': model.value}]

    AVAILABLE_CUSTOMER_FLOW_AND_SIZING_MODELS: List[dict] = []

    for model in CustomerFlowAndSizingModels:
        AVAILABLE_CUSTOMER_FLOW_AND_SIZING_MODELS += [{'model_type': model.name, 'model_name': model.value}]

    current_time = datetime.datetime.now()

    LOG_FILENAME: str = 'train_a_model_API_Log_' + str(current_time.day) + '_' + str(current_time.month) + '_' + str(current_time.year) + '.log'
