""" 
	...
	APIs:
		...
		/customer/flow/models/{model_id}/forecast
        /sizing/models/{model_id}/forecast
        /customer/flow/and/sizing/models/{model_id}/forecast
...
# custom imports
from app.models.forecast import ForecastIn, ForecastOut, ForecastingModels, CustomerFlowModels, SizingModels, CustomerFlowAndSizingModels, TypeForecast, MessageLevel
...
async def prophet_forecast(model_id: int, params: ForecastIn, model_storage_name: str, type_of_forecast_dir: str) -> ForecastOut:
    """
    ...
    try:
        fin = open(settings.MODELS_DIR + client_id + '\\' + type_of_forecast_dir + '\\' + model_storage_name, 'r')
...
async def mlp_forecast(model_id: int, params: ForecastIn, model_storage_name: str, forecast_period: int, type_of_forecast_dir: str) -> ForecastOut:
    """
    ...
    # load model
    mlp_model = await load_model(params.client_id, model_id, settings.MODELS_DIR + client_id + '\\' + type_of_forecast_dir + '\\' + model_storage_name)
...
async def hgbr_forecast(model_id: int, params: ForecastIn, model_storage_name: str, forecast_period: int, type_of_forecast_dir: str) -> ForecastOut:
    """
...
async def mlp_2_variables_forecast(model_id: int, params: ForecastIn, model_storage_name: str, forecast_period: int, type_of_forecast_dir: str) -> ForecastOut:
    """
...
async def RandomForestRegressor_forecast(model_id: int, params: ForecastIn, model_storage_name: str, forecast_period: int, type_of_forecast_dir: str) -> ForecastOut:
    """
    ...
    # load model
    rfrgr_model = await load_model(params.client_id, model_id, settings.MODELS_DIR + client_id + '\\' + type_of_forecast_dir + '\\' + model_storage_name)
...
async def get_model_forecast(model_type: str, model_id: int, params: ForecastIn, model_storage_name: str, forecast_period: int, type_of_forecast: str) -> Union[ForecastOut, None]:
    """
	Gets a forecast according to the model type.
	"""

    type_of_forecast_dir = 'customer_flow'

    if type_of_forecast == TypeForecast.Sizing.name:
        type_of_forecast_dir = 'sizing'
    elif type_of_forecast == TypeForecast.CustomerFlowAndSizing.name:
        type_of_forecast_dir = 'customer_flow_and_sizing'

    try:
        if model_type == ForecastingModels.MLPRegressor and type_of_forecast == TypeForecast.CustomerFlowAndSizing.name:
            return await mlp_2_variables_forecast(model_id, params, model_storage_name, forecast_period, type_of_forecast_dir)
        elif model_type == ForecastingModels.MLPRegressor:
            return await mlp_forecast(model_id, params, model_storage_name, forecast_period, type_of_forecast_dir)
        elif model_type == ForecastingModels.RandomForestRegressor:
            return await RandomForestRegressor_forecast(model_id, params, model_storage_name, forecast_period, type_of_forecast_dir)
        elif model_type == ForecastingModels.HistGradientBoostingRegressor:
            return await hgbr_forecast(model_id, params, model_storage_name, forecast_period, type_of_forecast_dir)
        elif model_type == ForecastingModels.Prophet:
            return await prophet_forecast(model_id, params, model_storage_name, type_of_forecast_dir)
    except Exception as e:
...
@router.post('/customer/flow/models/{model_id}/forecast', response_model=ForecastOut)
async def get_customer_flow_forecast(model_id: int, params: ForecastIn, request: Request, db: Session = Depends(get_db)) -> ForecastOut:
    """
    Forecast ustomer flow.
    """

    type_of_forecast = TypeForecast.CustomerFlow.name
    ...
    if not model:
        log_message(f'Forecast: client {params.client_id} model {model_id}. Model not found for client!', MessageLevel.Critical)
        raise HTTPException(status_code=404, detail=f'Model {model_id} not found for client {params.client_id}!')

    valid_type = False
    for fmodel in CustomerFlowModels:
        if model.type == fmodel.name:
            valid_type = True

    if not valid_type:
        log_message(f'Forecast: client {params.client_id} model {model_id}. Model {model.type} invalid type!', MessageLevel.Critical)
        raise HTTPException(status_code=404, detail=f'Client {params.client_id} model {model_id} type {model.type} invalid!')

    if model.type_forecast != type_of_forecast:
        log_message(f'Forecast: client {params.client_id} model {model_id}. Model {model.type}, {model.type_forecast} model invalid type of forecast!', MessageLevel.Critical)
        raise HTTPException(status_code=404, detail=f'Client {params.client_id} model {model_id} type {model.type} invalid type of forecast!')

    if model.type != ForecastingModels.Prophet:
        if model.forecast_period != params.forecast_period:
            log_message(f'Forecast: client {params.client_id} model {model_id}. Unexpected forecast period: received {params.forecast_period} expecting {model.forecast_period}!', MessageLevel.Critical)
            raise HTTPException(status_code=400, detail=f'Unexpected forecast period: received {params.forecast_period} expecting {model.forecast_period}!')
    ...
    if model.type == ForecastingModels.MLPRegressor:
        if len(keys) != 2:
            log_message(f'Forecast: client {params.client_id} model {model_id}. Unexpected number of keys in input data structure {keys} ({len(keys)}) expected (2)!', MessageLevel.Critical)
            raise HTTPException(status_code=400, detail=f'Unexpected input data structure!')
        if 'ds' not in keys or 'y' not in keys:
    ...
    forecast = await get_model_forecast(model.type, model_id, params, model.storage_name, model.forecast_period, type_of_forecast)
...
@router.post('/sizing/models/{model_id}/forecast', response_model=ForecastOut)
async def get_sizing_forecast(model_id: int, params: ForecastIn, request: Request, db: Session = Depends(get_db)) -> ForecastOut:
    """
    Forecast sizing.
    """

    type_of_forecast = TypeForecast.Sizing.name
    ...
    if model.type == ForecastingModels.MLPRegressor:
        if len(keys) != 2:
            log_message(f'Forecast: client {params.client_id} model {model_id}. Unexpected number of keys in input data structure {keys} ({len(keys)}) expected (2)!', MessageLevel.Critical)
            raise HTTPException(status_code=400, detail=f'Unexpected input data structure! Unexpected number of keys in input!')
        if 'ds' not in keys or 'y' not in keys:
            log_message(f'Forecast: client {params.client_id} model {model_id}. Unexpected input data structure (expected key (ds, y) not found) {keys}!', MessageLevel.Critical)
            raise HTTPException(status_code=400, detail=f'Unexpected input data structure! Bad input parameter names!')
    elif model.type == ForecastingModels.RandomForestRegressor:
        if len(keys) != 2:
            log_message(f'Forecast: client {params.client_id} model {model_id}. Unexpected number of keys in input data structure {keys} ({len(keys)}) expected (2)!', MessageLevel.Critical)
            raise HTTPException(status_code=400, detail=f'Unexpected input data structure! Invalid number of parameters!')
...
@router.post('/customer/flow/and/sizing/models/{model_id}/forecast', response_model=ForecastOut)
async def get_customer_flow_and_sizing_forecast(model_id: int, params: ForecastIn, request: Request, db: Session = Depends(get_db)) -> ForecastOut:
    """
    Forecast customer flow and sizing.
    """

    type_of_forecast = TypeForecast.CustomerFlowAndSizing.name
    ...
    valid_type = False
    for fmodel in CustomerFlowAndSizingModels:
        if model.type == fmodel.name:
            valid_type = True
