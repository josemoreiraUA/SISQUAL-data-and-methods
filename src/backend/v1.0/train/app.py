...
train_app = FastAPI(title='MLTrain', description='MLTrain Integration API', version='1.0', dependencies=[Depends(has_authorization)])
...
def get_task_state(task_id: int, db: Session = Depends(deps.get_db)):
    task = crud.get_task(db, task_id)
    if not task:
        raise HTTPException(
		        status_code=404, detail=f'Task {task_id} not found'
